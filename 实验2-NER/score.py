...
class SeqEntityScore(object):
    ...
    def get_result(self):
        ...
        for label, count in real_counter.items():
            ...
            result[label] = {"Precision": round(precision, 4), 'Recall': round(recall, 4), 'F1': round(f1, 4),'support':real_count}
        ...
        result["Total"] = {"Precision": round(precision, 4), 'Recall': round(recall, 4), 'F1': round(f1, 4),'support':real_total_count}


        return result
    
    def get_entities_bmeo(self, seq):
        entities = []
        entity = [-1, -1, -1]
        for indx, tag in enumerate(seq):
            ...
            if tag.startswith("B-"):
                ...
                if indx == len(seq) - 1:
                    entities.append(entity)
            elif tag.startswith('M-') and entity[1] != -1:
                _type = tag.split('-')[1]
                if _type == entity[0]:
                    entity[2] = indx
                if indx == len(seq) - 1:
                    entities.append(entity)
            elif tag.startswith('E-') and entity[1] != -1:
                _type = tag.split('-')[1]
                if _type == entity[0]:
                    entity[2] = indx
                    entities.append(entity)
    ...
    def report_scores(self, result):
        ...
        """
        # 打印表头
        header_format = '{:>15s}  {:>9} {:>9} {:>9} {:>9}'
        header = ['precision', 'recall', 'f1-score', 'support']
        print(header_format.format('', *header))

        row_format = '{:>15s}  {:>9.4f} {:>9.4f} {:>9.4f} {:>9}'
        ...
        for tag in self.result.keys():
            ...
            print(row_format.format(
                tag,
                self.result[tag]["Precision"],
                self.result[tag]["Recall"],
                self.result[tag]["F1"],
                self.result[tag]["support"]
            ))
...
class SingleClassificationScore(object):
    ...
    def update(self, pred_labels, real_labels):
        ...
        pred_labels = [label_id for label_id in pred_labels]
